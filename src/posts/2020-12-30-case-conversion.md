...
I got fed-up Googling case conversion utilities. The useful ones I found ([Voca](https://vocajs.com) and [change-case](https://www.npmjs.com/package/change-case)) both require an environment that allows me to use npm.

But as I was using Vanilla JavaScript, I did not want to include any toolchains in this project, and I'm reluctant to send an asynchronous request to download a module just for case-conversion.
...
I don't use these two cases even though I know they exist.
...
So I began by searching for the capital letters which denote the start of a new word (for `camelCase` and `PascalCase`). The easiest way is to loop through each letter and run a simple `/[A-Z]/` regex. This regex searches for any letter that's between A and Z.
...
Then I lowercased the capital letters and added a space in front of them. (It didn't matter whether I added space or `_`, both were fine since I was going to replace them with `-` later).
...
Note: I'm pretty sure there's a regex that can do these three steps in one go. It probably uses capturing groups and substitution, which I'm not familiar with. I didn't try it since I didn't have the time or energy to research further. If you know a good regex for this, let me know!
...
- 2. Second replace uses `/\s+/` to replace all spaces with `-`. The `+` indicates "one or more", so it matches the two spaces in `title case`.
...
But I realized I could use my `toKebab` function to convert everything into `kebab-case` first. This takes advantage of the work that I've already done.

From a performance standpoint, I was initially against this idea because it seemed like a "waste of resources" to run another function first. But I realized I was being idealistic. From a practical standpoint, it doesn't have significant impact on performance since the operations are really fast.
...
It's equally easy to convert all cases into Sentence case. Once again, I started by converting cases into `kebab-case`.
...
Finally, I wanted to be able to convert any string back into camel case.
...
At this point, I only needed to convert `kebab-case` into `camelCase`.

I did this by:

1. Splitting the word at each `-`. This created an array of words.
2. Looping through the array and capitalizing the first letter if it's not the first word.
<!-- Point 2 is a little unclear to me though -->
