...
As you can see from the example above, when you compare a string of `24` vs the number 24, `
...
The strange thing is, `a
 - = b` is always going to be false. Why?
...
Is it a surprise that `a
 - = b` is always going to be false now?
...
In this case, `a
 - = b` evaluates to true because `b` now points to the same reference as `a`.
