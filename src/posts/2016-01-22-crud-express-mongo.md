...
<figure>
  ![](/images/2016/01/crud-express-mongo.png)
</figure>
...
1. You aren't afraid of typing commands into a shell. Check out [this article](/blog/fear-of-command-line/) if you currently are.
...
You should get a version number if you have Node installed. If you don't, you can install Node either by downloading the installer from [Node's website](https://nodejs.org/) or downloading it through package managers like [Homebrew](https://brew.sh/) (Mac) and [Chocolatey](https://chocolatey.org/) (Windows).
...
<figure>
  ![](/images/2016/01/exp-package-json.png)
</figure>
...
I'm going to start writing in ES6 code and show you how to convert to ES6 along the way as well. First off, I'm replacing `function()` with an [ES6 arrow function](https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Functions/Arrow_functions). The below code is the same as the above code:
...
<figure>
  ![](/images/2016/01/get-hello-world.png)
</figure>
...
<figure>
  ![](/images/2016/01/get-index.png)
</figure>

This is how Express handles a **GET** request (**READ** operation) in a nutshell.

At this point, you probably have realized that you need to restart your server whenever you make a change to `server.js`. This is process is incredibly tedious, so let's take a quick detour and streamline it by using a package called [nodemon](https://nodemon.io/).
...
<figure>
  ![](/images/2016/01/post-log-req-body.png)
</figure>
...
The next part is to get the correct link to our database. Most people store their databases on cloud services like [mLab](https://mlab.com/) (formerly MongoLab). We're going to do same as well.
...
<figure>
  ![](/images/2016/01/mongolab-dbuser.png)
</figure>
...
<figure>
  ![](/images/2016/01/mongo-dburl.png)
</figure>
...
Next, we want to start our servers only when the database is connected. Hence, let's move `app.listen` into the `connect` method. We're also going to create a `db` variable to allow us to use the database when we handle requests from the browser. (**Note:** Mongo's `.connect()` used to return the database directly, but they [changed that](https://github.com/mongodb/node-mongodb-native/blob/3.0.0/CHANGES_3.0.0.md#api-changes) in 3.0.0. The code is updated to use the newer method here.)
...
<figure>
  ![](/images/2016/01/mongodb-collection.png)
</figure>
...
<figure>
  ![](/images/2016/01/mongo-cursor.png)
</figure>
...
<figure>
  ![](/images/2016/01/mongodb-get.png)
</figure>
...
I've written extensively about the how and why of template engines in a [separate post](/blog/nunjucks-with-gulp/). You might want to check it out if you have no idea what template engines are. I personally use (and recommend) Nunjucks as my template engine of choice. Feel free to check out the post to find out why.
...
<figure>
  ![](/images/2016/01/get-rendered.png)
</figure>
...
There are two more operations to go, but we'll leave it to the [next post](/blog/crud-express-and-mongodb-2). Catch you there!
