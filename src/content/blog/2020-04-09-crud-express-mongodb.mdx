...
<Image src="/src/assets/2020/crud/crud-express-mongo.png" alt="" />
...
<Image
  src="/src/assets/2020/crud/looks.png"
  alt="Add CSS to make the app look better."
/>

Free free to check out the [demo][5] before continuing with this tutorial.

<Callout>
  This article is LONG! Remember to **grab the source code by leaving your name
  and email address in [this form][6]**. I'll also send you this article in PDF
...
<Image src="/src/assets/2020/crud/node-version.png" alt="Node version" />
...
<Image src="/src/assets/2020/crud/npm-init.png" alt="npm init." />
...
<Image
  src="/src/assets/2020/crud/node-log.png"
  alt="Logged May Node be with you."
/>
...
Run `npm install express` command in your command line.

<Callout>
  What you install using npm will be added as a `dependency` in `package.json`.
  It's important to know these dependencies because `npm` can retrieve
  dependencies with another `npm install` command when you need it later.
</Callout>
...
<Image
  src="/src/assets/2020/crud/exp-dependency.png"
  alt="Saved express as a dependency"
/>
...
<Image src="/src/assets/2020/crud/cannot-get.png" alt="Cannot get /." />
...
<Image src="/src/assets/2020/crud/get-hello-world.png" alt="" />
...
<Image src="/src/assets/2020/crud/get-index.png" alt="The HTML file." />
...
<Callout>
  We use a `--save-dev` flag here because we only use Nodemon when we are
  developing stuff. We won't use Nodemon on an actual server. `--save-dev` here
  adds Nodemon as a `devDependency` in the `package.json` file.
</Callout>

<Image
  src="/src/assets/2020/crud/nodemon-dev-dep.png"
  alt="Saved Nodemon as a dev dependency."
/>
...
<Image
  src="/src/assets/2020/crud/nodemon-server.png"
  alt="Uses npm run dev to run nodemon server.js"
/>

<Callout>
  Node v18.11 added support for watch mode. This watches for changes and
  restarts the server so you don't have to use Nodemon. You can simply run `node
  --watch server.js` instead.
</Callout>
...
Restart your server (hopefully you've set up Nodemon or are using --watch so it restarts automatically) and refresh your browser. Then, enter something into the `<form>` element and submit the form. Next, look at your command line. You should see `Hellooooooooooooooooo!` in your command line.

<Image
  src="/src/assets/2020/crud/post-log.png"
  alt="Logs helloooo in the command line."
/>
...
Turns out, Express doesn't handle reading data from the `<form>` element on it's own. We have to use middleware to gain this functionality. It will help to tidy up the `request` object before we use it. Express lets us use middleware with the `use` method.
...
The `urlencoded` method on express tells express to extract data from the `<form>` element and add them to the `body` property in the `request` object.
...
<Image
  src="/src/assets/2020/crud/post-log-req-body.png"
  alt="Logs the request body."
/>
...
<Callout>
  I normally use [Mongoose][15] (which is a framework for MongoDB) when I use
  MongoDB. I'll teach you how to use basic MongoDB in this article. If you want
  to learn Mongoose, consider reading [my article on Mongoose][16].
</Callout>
...
<Callout>
  You can also create a database on your computer for development work. Read
  "[How to setup a local MongoDB Connection][18]" for instructions.
</Callout>
...
<Image
  src="/src/assets/2020/crud/atlas-create-org.png"
  alt="Creates an Atlas Organization"
/>

You also need to select a cloud service. Go ahead with MongoDB Atlas in this case.

<Image
  src="/src/assets/2020/crud/atlas-select-cloud.png"
  alt="Selects a cloud service."
/>
...
<Image
  src="/src/assets/2020/crud/atlas-projects.png"
  alt="Atlas project screen."
/>
...
<Image src="/src/assets/2020/crud/atlas-cp.png" alt="Creating a new project." />

Next, you will need to name your project. Call it anything you want. I'm going to call this `star-wars`.

<Image
  src="/src/assets/2020/crud/atlas-cp-name-project.png"
...
<Image
  src="/src/assets/2020/crud/atlas-cluster.png"
  alt="Create Cluster Screen."
/>

Click on "Build a Cluster". You should see this screen:

<Image src="/src/assets/2020/crud/atlas-cc.png" alt="Selecting a cluster" />
...
<Image
  src="/src/assets/2020/crud/atlas-cc-m0.png"
  alt="Configured the cluster."
/>

Click on Create cluster next. You should see "Your cluster is being created".

<Image
  src="/src/assets/2020/crud/atlas-cc-creating.png"
  alt="Creating cluster."
/>

You have to wait for approximately 5 minutes for the cluster creation. When the cluster is ready, you'll see this:

<Image src="/src/assets/2020/crud/atlas-cc-ready.png" alt="Cluster ready" />
...
<Image src="/src/assets/2020/crud/atlas-connect.png" alt="Connect button." />

A modal should pop up.

<Image
  src="/src/assets/2020/crud/atlas-connect-modal.png"
...
<Image
  src="/src/assets/2020/crud/atlas-connect-add-ip.png"
  alt="Adds IP address."
/>
...
<Image
  src="/src/assets/2020/crud/atlas-connect-mongodb-user.png"
  alt="MongoDB User."
/>
...
<Image
  src="/src/assets/2020/crud/atlas-connect-method.png"
  alt="Choosing a connection method."
/>

<Image
  src="/src/assets/2020/crud/atlas-connection-string.png"
  alt="Copy the connection string."
/>
...
<Callout>
  The `test` in the connection string points to a `test` database. You would
  need to replace `test` with the name of your database if you use Mongoose. You
  can leave it as `test` if you use MongoClient like what we're doing in this
  tutorial.
</Callout>
...
<Image
  src="/src/assets/2020/crud/connect-no-deprecation-warnings.png"
  alt="Connection without deprecation warning."
/>
...
<Image src="/src/assets/2020/crud/post-result.png" alt="Post result." />
...
<Image
  src="/src/assets/2020/crud/atlas-collections.png"
  alt="Collections in MongoDB Atlas."
/>

You should see a document in your database. (Each database entry is called a document).

<Image
  src="/src/assets/2020/crud/atlas-collection-view.png"
  alt="Documents in MongoDB Atlas."
/>

If you go back to the Browser, you'll see it's still trying to load something.

<Image
  src="/src/assets/2020/crud/post-loading.png"
  alt="Browser still trying to load a page."
/>
...
<Image
  src="/src/assets/2020/crud/post-redirected.png"
  alt="Redirected. Browser is no longer waiting to load something."
/>
...
<Image
  src="/src/assets/2020/crud/cursor.png"
  alt="Cursor object from MongoDB"
/>
...
<Image src="/src/assets/2020/crud/mongodb-get.png" alt="" />
...
<Image src="/src/assets/2020/crud/get-ejs.png" alt="Renders an ejs file." />
...
<Image
  src="/src/assets/2020/crud/ejs-quotes.png"
  alt="Rendered quotes in EJS."
/>
...
<Image
  src="/src/assets/2020/crud/ejs-rendered-quotes.png"
  alt="Rendered quotes with EJS."
/>
...
<Image
  src="/src/assets/2020/crud/put-ejs.png"
  alt="Add update section to HTML."
/>
...
Our server doesn't accept JSON data yet. We can teach it to read JSON by using express's `json` middleware.
...
<Image src="/src/assets/2020/crud/put-body.png" alt="Put request body" />
...
<Image src="/src/assets/2020/crud/put-result.png" alt="Put result." />

If you refresh the browser, you should see Darth Vader's quote as the first quote.

<Image
  src="/src/assets/2020/crud/put-check.png"
  alt="Checking the HTML after a PUT request."
/>

<Callout>
  Does the `findOneAndUpdate` look complicated to you? Well, It IS complicated.
  This is why I use Mongoose instead of MongoDB. You can find out more about
  Mongoose in [this article][28].
</Callout>
...
<Image
  src="/src/assets/2020/crud/put-response.png"
  alt="Response from the server."
/>

<Callout>
  I wrote an [article on the Fetch API][29] if you're wondering why we need two
...
<Callout>
  ...
  through my [Learn JavaScript][30] course. I even teach you how make your
...
<Image
  src="/src/assets/2020/crud/delete-ejs.png"
  alt="Adds delete button to the index.ejs file."
/>
...
Now, when you click the delete button, the browser will send the _DELETE_ request through Fetch to our Express server. Then, the server responds by sending either an error or a message back.
...
<Image
  src="/src/assets/2020/crud/delete-result.gif"
  alt="Message if no quotes can be deleted."
/>
