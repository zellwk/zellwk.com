...
Now, all the three code examples above assumes you’re using an unnamed container, which is sufficient for most use cases.
...
Since your `.grid` is the container, container query units will be based on the height and width of the `.grid`. This is probably not wah tyou want, because it can produce unintended (huge) sizes like the one you see below.
...
But because the `.grid` is container, `cqi` draws from the width of the `.grid` (which is almost the same as the browser viewport in this case). So `cqi` produced an oversized image.

This blunders can be prevented if we used the HTML structure I recommend — because container query units will draw its width from the grid item instead.
