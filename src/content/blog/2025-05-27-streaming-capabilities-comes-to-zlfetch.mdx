...
Even for chunked encoding, you can _actually_ receive the data in your browser as chunks! To make that happen, we can't parse the stream in advance inside zlFetch. But we've given you a `readStream` helper to, you know, deal with the stream decoding stuff.
